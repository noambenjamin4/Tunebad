...
7. **CSP.** `middleware.ts` sets a per-request nonce CSP: `script-src 'self' 'nonce-…' 'strict-dynamic' 'wasm-unsafe-eval'` (`'unsafe-eval'` only in dev for React Refresh), `object-src 'none'`, `frame-ancestors 'none'`, `connect-src 'self' https://*.supabase.co wss://*.supabase.co`. Plus `X-Content-Type-Options: nosniff`, `Referrer-Policy: no-referrer`, `X-Frame-Options: DENY`.
